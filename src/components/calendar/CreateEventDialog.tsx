...
export default function CreateEventDialog({ isOpen, onClose, onSave, initialDate = new Date(), initialEvent = null }: CreateEventDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Store the Unix timestamps for recurrence fields only
    const startTimestamp = Math.floor(startDate.getTime() / 1000);
    const endTimestamp = Math.floor(endDate.getTime() / 1000);
    const eventData = {
      id: eventId, // Include the ID for editing existing events
      title,
      start: startDate, // Use Date object directly to match ScheduleEvent interface
      end: endDate,     // Use Date object directly to match ScheduleEvent interface
      isAllDay,
      type: eventType,
